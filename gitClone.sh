...
if [ $full_url_y_n = "y" ]
    then
        ...
        select newBranch in dev_branch main_branch custom quit; do
            case $newBranch in
                dev_branch)
                ...
                main_branch)
                    ...
                    break 2;;
                custom)
                    printf "${BLUE}Enter branch name: ${RESET}"
                    read branch_name
                    printf "Cloning into repo: ${UCyan}${full_git_url}${RESET}\n"
                    git clone $full_git_url
                    printf "${BGREEN}changing directory => ${UCyan}${full_url_repo_name}${RESET}\n"
                    cd $full_url_repo_name
                    printf "${BGREEN}Creating branch: ${UCyan}${branch_name}${RESET}\n" 
                    git branch $branch_name
                    git checkout $branch_name
                    printf "\n${BGREEN}Finished cloning into ${UCyan}${full_url_username}/${full_url_repo_name}${RESET} ${BGREEN}and selecting branch ${UCyan}${branch_name}${RESET}\n"
                    printf "${BGREEN}Installing dependencies...${RESET}\n"
                    npm install
                    printf "\n${BGREEN}Navigate to new repo by running ${UCyan}cd ./${full_url_repo_name}${RESET}"
                    break 2;;
...
select branch in dev_branch main_branch custom quit; do
    case $branch in
        dev_branch)
            printf "${BGREEN}Selected ${UCyan}${dev_branch}${RESET} ${BGREEN}branch\n"
        ...
        custom)
            ...
            git branch $branch_name
            git checkout $branch_name
            printf "\n${BGREEN}Finished cloning into ${UCyan}${git_url}${RESET} ${BGREEN}and selecting branch ${UCyan}${branch_name}${RESET}\n"
            printf "${BGREEN}Installing dependencies...${RESET}\n"
            npm install
            printf "\n${BGREEN}Navigate to new repo by running ${UCyan}cd ./${git_repo_name}${RESET}"
            break 2;;
